...
- **Windows checklist subset (non-flaky, PR-ready)** — `jwebgen --setup --dry-run`, `jwebgen --new myapp --yes`, `jwebgen --servlet`, `jwebgen --jsp`, `jwebgen --status` with a fake `TOMCAT_HOME` (CI only asserts Windows hints: no `systemctl` in output; deployment line may be “unknown” if the fake tree is not a full Tomcat install).
