...
if (!statusOut.includes('Deployment: present')) {
  throw new Error('status output missing deployment marker (fake Tomcat layout must satisfy probeApacheTomcatHome)');
}
if (!statusOut.includes('http://localhost:8080/globalapp/')) {
  throw new Error('status output missing default Tomcat app URL');
}
